...
def min_cover(U, max_k, Dset):
    """精确求覆盖 U 所需最少新字数（迭代加深）；若 >max_k 则返回 max_k+1。"""
...
def run(d, limit=None, verbose=True, i1_lo=0, i1_hi=119, imod=None):
    t0 = time.time()
    stats = {"d": d, "tested": 0, "pruned_size": 0, "pruned_pack": 0, "need_lower": 0,
             "best_defect": None, "found": [], "tau_hist": {}, "u_hist": {},
             "min_margin_vs_dminus1": None, "min_margin_vs_dplus1": None,
             "near_miss_candidates": [], "cap_hits": 0}
    cap = 11 * (d - 1)
    n = 0
    for D in itertools.combinations(range(120), d):
        if D[0] < i1_lo:
            continue
        if imod and D[0] % imod[1] != imod[0]:
            continue
        if D[0] > i1_hi:
        ...
        for i in D:
            u1 |= L1[i]
        stats["tested"] += 1
        if u1.bit_count() > cap or dist_ge3_packing(u1, d - 1) > d - 1:
            stats["pruned_size"] += 1
            continue
        U = U_of(D)
        if U == 0:
        ...
        if tau > cap_tau:
            stats["cap_hits"] += 1
        stats["tau_hist"][str(tau)] = stats["tau_hist"].get(str(tau), 0) + 1
        m1 = tau - (d - 1)
        mp1 = tau - (d + 1)
        if stats["min_margin_vs_dminus1"] is None or m1 < stats["min_margin_vs_dminus1"]:
            stats["min_margin_vs_dminus1"] = m1
        if stats["min_margin_vs_dplus1"] is None or mp1 < stats["min_margin_vs_dplus1"]:
            stats["min_margin_vs_dplus1"] = mp1
        if tau <= d + 1 and len(stats["near_miss_candidates"]) < 50:
            stats["near_miss_candidates"].append({"D": list(D), "tau": tau, "U_size": pc2})
        if tau <= d - 1:
            stats["found"].append({"D": list(D), "A_size": tau})
            print(f"  ★ d={d} 发现 119-码！D={D} |A|={tau}", flush=True)
        else:
            stats["need_lower"] += 1
...
if __name__ == "__main__":
    args = sys.argv[1:]
    shard = None
    if args and args[0].startswith('mod'):
        d, k, nsh = (int(x) for x in args[0][3:].split(':'))
        res = run(d, i1_lo=0, i1_hi=119, imod=(k, nsh))
        json.dump([res], open(f'b1b_result_d{d}_mod{k}of{nsh}.json','w'), indent=1)
        print('已写 mod shard json'); sys.exit(0)
