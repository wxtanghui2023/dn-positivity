...
ne=0;nc=0;ns=0;nu=0;disc=[];minm=None
while st:
    b=st.pop();ne+=1
...
print(f"   认证最小余量 = {minm:.6e}")
if disc:
    fmax=max(f for f,_ in disc)
    print(f"   丢弃箱最远角距离 max = {fmax:.12f}  ⟹ ≤ {DISC_R}? {fmax<=DISC_R} ✓")
json.dump(dict(TC=TC,Fz0=Fz0,DISC_R=DISC_R,ne=ne,nc=nc,nd=len(disc),ns=ns,nu=nu,min_margin=minm,
