...
## AMEND-20（2026-09-24）：族字面检索硬门（防重复投资）

**背景（实例）**：`P2` 线在族 `(1,t,t,-s,-s)` 上做了 `P2(ii)→P2(iv)`＋四点扫描＋真实构造，事后发现该族正是 **Johnson–Marijuán–Pisonero 2017**（LAA 512,129–135）的研究对象（**该文当时已在档案内、且其定理已被使用过**），且 **Marijuán 2023**（LAA）已给出“`a ≤ (√5−1)/4` 时恒可对称实现（常对角）”——我方点 `t=1/4` **被直接覆盖**。属 **AMEND-9 §5「形态变化未重跑文献核查」的第二实例**。

**触发条件（任一即触发；与入口核查相互独立）**：
1. 出现**具名谱族 / 矩阵族 / 参数族**（例：`(1,a,a,-(a+d),-(a+d))`）；
2. 归一化或参数化方式改变；
3. 进入新的子区域（条带 / 边界 / 边缘层 / 高迹或低迹分区）；
4. 工作性质从"分类"转入"构造 / 攻击 / 渐近分析"。

**强制动作（缺一即视为违规）**：
1. 以**族字面式**检索（含符号、括号、负号；**不得**只搜主题词如 “symmetric nonnegative inverse eigenvalue problem”）；
2. 文档内必须有**【形态绑定文献核查】**小节，三列对照：`查询式 ⇄ 命中文献 ⇄ 覆盖判定`；
3. 若命中文献对**当前参数点**已给出可实现 / 不可实现结论 ⟹ **STOP**，不得进入构造或渐近分析；
4. 保留原始查询与命中原句（摘要级须标注“摘要级”）。

**与 AMEND-14 的分工**：AMEND-14 防**误杀**（禁类别式拒绝）；AMEND-20 防**重复投资**（逐条字面覆盖）。两者不可互相替代。

**违规认定**：未做族字面检索而投入 ≥2 轮工作 ⟹ 记为流程违规，须在阶段报告中列出**成本范围**。

**本次成本范围**：`P2(ii)`–`P2(iv)` ＋ 四点最小扫描 ＋ `W=0` 三阶 ＋ 真实局部构造 ＋ 引理抽象（≈ 8 轮），全部落在已覆盖区域内。

## AMEND-21（2026-09-24）：开题 Gate 0 ——“三合一未覆盖”＋“未覆盖≠未知”禁令

**本修正与 AMEND-20 配套**：AMEND-20 规定**何时必须重查**（形态触发）与**如何查**（族字面式＋三列覆盖表）；AMEND-21 规定**什么才算通过**（Gate 0）与**一条禁令**。

### Gate 0（进入大规模推导的前置门，三条件必须**同时**满足）

1. **现有论文未覆盖**：目标点/区域无已发表的肯定或否定结论；
2. **既有充分/必要条件未覆盖**：**必须**检索该问题**所有已知充分条件与必要条件**（含相邻参数化、等价归一化下的改写形式），确认目标点不在其中；
3. **等价谱型/矩阵族文献未覆盖**：以**等价参数化**（不同符号体系、不同归一化、缩放/平移等价类）再检索一遍，确认无等价对象已被研究。

> 只有 (1)∧(2)∧(3) 全为“未覆盖”，才允许进入大规模计算。

### 禁令（硬性）

$$\boxed{\text{“某篇论文未覆盖”}\ \textbf{绝不能}\ \text{作为“该区域未知”的证据}}$$

即：**不得**由“最新论文的方法不适用”推出“这里值得攻击”。必须进一步查该论文**引用的既有充分条件**与**相邻参数化文献**。

### 身份切换（形态冻结即复审）

当研究对象从抽象机制**冻结**为一个可识别的标准数学对象时（明确谱型／方程族／矩阵类／猜想／优化问题／参数区域），**立即**执行一次完整复审：

```
精确对象名 + 参数化 + 等价参数化 + 充分/必要条件 + 最近文献
```

**本次违规**：对象在获得明确谱型 `(1,t,t,-s,-s)` 时未触发该复审，导致在已覆盖区域内投入 `P2(ii)→真实构造`（≈8 轮）。

### 违规成本记录（本线）

| 项目 | 内容 |
|------|------|
| 违规类型 | AMEND-9 §5（形态变化未重跑）× AMEND-21（Gate 0 未执行） |
| 成本范围 | `P2(ii)`–`P2(iv)` ＋ 四点最小扫描 ＋ `W=0` 三阶 ＋ 真实局部构造 ＋ 引理抽象（≈8 轮） |
| 保留 | 方法/复核资产（见 `M03-P2-SNIEP-FINAL-STAGE-REPORT.md` §3） |
| 作废 | 新性主张（族内构造、`R\setminus W` 内实现、"`(√5−1)/4` 新阈值"） |

## AMEND-22（2026-09-24）：候选推荐前置"对象指纹预审"（选择阶段门）

**背景（实例）**：`Mt07` 被选为下一课题。事后判定：**选择阶段误判**，而非"Gate 0 做得不够"。三点错因：
1. 把 **"Gate 0 成本低"** 误当 **"值得开题"** —— 易查清 ≠ 值得投入；
2. **未先服从现成 census 状态**（`C07 CLOSED`、`M03 CLOSED`、`C06`、Zone-B WATCH），而**从概念层重新发明候选**；
3. **未在推荐前执行"对象指纹预审"** —— `R(C_4,K_{1,n})` 本应立刻触发高重复警报（经典 Ramsey 族 ＋ DS1.18 ＋ 已有 2025 综述 ＋ 等价 C₄-free 图）。

**责任划分（如实记录）**：操作方选了该候选；**执行方在"准备度排序"中把 `Mt07` 排第一而未做指纹预审** —— 双方各有其责，制度须同时覆盖两处。

**硬规则（候选推荐前置门，先于 Gate 0）**：
1. **来源限定**：候选**必须**从现有 census/清单的**余项状态**中续选；**禁止**从概念层重新发明候选；
2. **指纹预审三项**（缺一不得作为"下一个"推荐）：
   - **家族类别**：是否属经典主战场（Ramsey / 编码分类 / 极值图 等）？
   - **已列清单**：是否已出现在已下载综述/规范表（如 `DS1.18`、Brouwer 表、Grassl 表）中？
   - **等价对象**：是否存在等价参数化（图/超图/设计/OA/Boolean/谱对象）且已被研究？
3. **判定表述**：命中任一项 ⟹ 标注 **高重复风险** 并**降序**（不得排第一）；三项全空才可推荐为"下一个"；
4. **与 Gate 0 的分工**：AMEND-22 = **选之前**（防推荐错对象）；AMEND-20/21 = **算之前**（防重复投资）。二者不互相替代。

**本次成本**：候选选择误判 → 消耗 `WP0`＋`G0` 两轮（未进入计算）。**正面确认**：Gate 0 确实阻断了更大的浪费，但这**不能**用于解释选择阶段的失误。

## AMEND-23（2026-09-24）：双轨入口架构（LANE-A / LANE-B 分轨＋闸门作用域）

**背景（已核验的实验）**：回溯把 `C07`（唯一实产出）投入现行四闸，得 `G1 PASS / G2 FAIL / G3 PASS / G4 WATCH` ⟹ **四闸会把 `C07` DROP**。定性：四闸**不是错误，而是作用域未声明** —— 它实质是 **LANE-B（新机制）专用**，却被当作普适入口使用；LANE-A（dent）**从未有对应闸门集**。这是"池为空"的架构原因。

### 双轨入口

```
具名独立来源 → 具体问题 → { LANE-A（dent） | LANE-B（mechanism） }
```

**LANE-A（dent）四闸**：① 明确记录缺口（**须含"权威记录源可得并已读"**，否则缺口仅算暂定）∧ ② 可机器验证 ∧ ③ 独立复核 ∧ ④ 非纯算力堆砌。
- 基准样本：`C07`（四闸全过）。
- 证书要求：可公开复验，并明确 **旧记录 / 改进值 / 证明对象 / 验证脚本**。
- **不要求**新机制或新不变量。

**LANE-B（mechanism）四闸**：沿用 `G1 独立问题 → G2 新量 → G3 可证明性 → G4 前沿新性`；**尤其保留 G2**（已成功杀掉 `C06/D03/M04/Au06/G06` 整类"开放但无新量"伪机会）。

### AMEND-22 作用域澄清

`AMEND-22` 约束 **"进入候选池之后不得偷换对象 / 重新发明对象"**，**不约束候选来源**。
⟹ `AMEND-22` 防重复投资；`HUNT-R3` 负责供给；**两者非竞争关系**。

### HUNT-R3（供给端恢复）

候选不再要求属于旧资产驱动 census；改为：
**具名独立来源**（Erdős 问题库／领域动态综述的未收割格／期刊问题栏／会议问题表）→ **具体问题** → **双轨分派**。

**分派规则（路由）**：
- 若问题形态为"已知但未闭合的记录缺口 + 可机器验证" ⟹ **LANE-A**；
- 若问题形态为"机制/新不变量/结构定理" ⟹ **LANE-B**；
- 两轨均须过 `AMEND-20/21`（防重复投资）后再入场。

## AMEND-24（2026-09-24）：判据翻转 —— "已被做出" 才是杀，**"有人在做" 不是**

**背景（本项目 20+ 轮的错）**：准入判据被设成"未覆盖 ∧ **未被占有** ∧ 非经典 ∧ 非算力"，导致 5 源全灭、75 条 + 数百格全 DROP。而 Astra 的十项结果（非 sofic 群构造／Connes 刚性反例／Ehrhart 尖锐界／多色 Ramsey 超指数下界／极值图论两个 Erdős 问题…）**恰好落在我判死的区域内** ⟹ 判据本身错位。

**⭐ 核心区分（本轮确立）**：
```
① 「有人在跑」= 竞争信息   ⟹ 不构成排除理由（用户 2026-09-24 明确）
② 「已经做出成果」= 真实杀 ⟹ 仅当 目标命题已被（可核验的）成果覆盖时成立
```
**旧判据错在**：把 ① 当 ② 用（"AI 在攻击"被当成规避信号，而证据是那里正在做**成**）。

**新准入判据（两条，不再要求"未覆盖"）**：
1. **可产出性**：能给出四类之一 —— (i) 构造 (ii) 反例 (iii) 界改进 (iv) 猜想解决；
2. **可验证性**：输出须**机器可验**（Lean 形式化，或显式证书 + 独立重解）。
3. **"谁在做"降为背景信息**；**算力不再是排除理由**（区分"平凡穷举"与"推导 + 验证"）。

**保留**：AMEND-20/21/22 仍作为**防对象偷换与防重复投资**的写作纪律，但**不得**用以否定一个"可产出 ∧ 可验证"的靶。

**本修正的直接后果**：多项先前判死属**误杀**（见 `docs/AMEND-24-reaudit-kill-vs-occupied.md`）。

## AMEND-25（2026-09-25）：`NEW-INFO GATE`（A–E）＋ zero-to-obstruction 同构屏

**背景**：Astra 十项（OpenAI 253 页论文集 ＋ 62 页推理回顾 ＋ Lean 仓库）带来的"新机制"逐条对照档案后，多数归约为既有骨架的换名。为使此类判断机械化，立此闸。

**Gate A**：**新变量不算**（new symbol ≠ novelty）。
**Gate B**：**新组合不算**（A+B+C 换顺序 ≠ novelty）。
**Gate C**：**新表示不算**（同一对象的新 representation ≠ novelty）。
**Gate D（硬门槛）**：必须写出新的**不可满足关系** 𝒪_new，满足 𝒪_new ⇏ 𝒪_old（不由旧约束推出），且对目标构型给出新限制 𝒪_new(ρ) ≤/≥ Φ(β,γ)。
**Gate E（同构检查）**：投入前先答"是否最终等价于"：① 局部量 = f(γ)；② 所有局部量共享 β；③ β = 1/2；④ positivity ⇔ RH；⑤ 谱实现 ⇔ RH。命中任一 ⟹ 归档，不开新线。

**⭐ 本轮新增（来自 Astra 同构审计）——`zero-to-obstruction 屏`**：
凡候选机制最终要求形如「**离轴 ⟹ 某固定对象退化／碰撞／坏约化**」的条件步骤，一律视为同一 residual GAP 的换名（见 `ZF-RPC-1`：该类"自然实例路线"已判 **CLOSED（人为并置）**，判词逐字："三类均无独立新来源；全部归约到同一个 residual GAP"），**除非另行提供该退化事件的独立来源**。不得以"codimension"等词作为新性证据（`IP-1-G0.2` 已列 codimension 为改名即 FAIL）。

**适用**：任何"新机制／新桥／新接口"候选，包括外部论文、AI 成果、跨领域类比；**Astra 批次整体据此降级为方法学参考** ✓

## AMEND-26（2026-09-25）：**能力索引制** —— 废止"未被占据的缝隙"判据

**起因（唐先生 2026-09-25 09:24）**："对于 RH，这不是新的，但同时也说明我们的资产可以处理的数学课题是可以非常丰富多彩的，而不是之前搜索筛选的那么有限几个，甚至根本筛选不出来。"

**诊断**：此前候选搜索失败**不是**因为资产贫乏，而是判据方向错了 —— 用**否定式判据**（"找没人占据的缝隙"）去筛**策展型清单**（清单上的 open 项天然是被人盯得最紧的项）⟹ 结构性近空。
**而同日 Astra 十项同构审计证明**：给定真实数学成果，我方资产能**逐条**给出精确判定（同构／包装／新线），即资产具备**跨领域的处理与判别能力**。

**制度变更**：
1. **废止**"未被占据的缝隙"作为主判据（不再以"没人做过"为入场条件）。
2. **改为能力索引制**：任何课题须先归入下列**能力类**，并**先声明交付物**（不要求新性）：
   - **K1 精确有限搜索 ＋ 证书**（例：`C07` 覆盖码格、`X1` `K(10,1)`、设计/拟阵分类格）
   - **K2 机械式同构／去包装审计**（即 `AMEND-25` `NEW-INFO GATE`；例：Astra 十项审计）
   - **K3 形式化**（Lean 可迁移证明骨架；例：`TLDC`）
   - **K4 表格／记录核验**（须附 `E4` 警戒：表龄 ≠ 数学缺口）
   - **K5 外部成果独立审计（新产品线）** —— AI 时代大量"宣称成果"需要**档案级同构判定**；试点 = OpenAI Astra 十项（本项目已完成一轮）
3. **交付物优先级**：证书 ＞ 审计结论 ＞ 形式化 ＞ 记录改善 ＞ "新机制"（`AMEND-25` Gate D 门槛最高）。
4. **不再要求**"文献新性"作为 K1/K2/K3/K5 类工作的入场条件；仅 `AMEND-25` 声称"新机制"时才要求。

**边界（诚实）**：本制度**不**声称我方能在任意领域产出新定理；已证实的是**审计／核验／有限计算／形式化**四类产品，以及**跨领域判别**能力。不得把"能判别"说成"能创造"。

## AMEND-27（2026-09-25）：**双空间解耦** —— RH 搜索 ／ 资产原生搜索

**起因（唐先生 09:26）**："这两个搜索空间应该解耦……以后即使 RH 连续几十条路线全部 CLOSED，也不等于这套资产'没有数学产出能力'……如果某个机制在一个独立问题上产生了新的定理/界/构造，它也应该被视为**实质性产出**，而不是'因为没接上 RH 所以无效'。"

**制度**：
- **空间 A（RH 搜索）**：维持并**加严** `AMEND-25`：`新信息源 → 𝒪_new → 对零点产生独立约束`；无独立 obstruction 不开线；形式漂亮不算理由。
- **空间 B（资产原生搜索）**：**完全不再要求接 RH**。每个机制须回答："**这套机制最自然、最强、最容易产生可验证突破的问题是什么？**" 允许图论／组合／编码／离散几何／数论／代数／复杂性／谱／群论／凸几何／以及此前未主动搜索的猜想。
- **解耦规则**：两空间的准入判据、交付物、账本**彼此独立**；空间 B 的成果**不得**被"未接 RH"降级。
...
### AMEND-28 补 §7（2026-09-25）：**E4-0 语义闸**（`F4-s` 故障模式）

**起因**：`B-LJCR-1` 用 CXS 指数界把 `DS(243,121,60,[3,9,9])` 判 `No` 并批量判 74 格；**自查发现越界** —— CXS 约束的是 **skew Hadamard** 差集，而 LJCR 数据库的定义（README 逐字）是**一般差集**，无 skew 条件；数据库自身 44 个 `Yes` 且 v 非素数幂（TPP／Singer 构造）即为冲突信号，追查 README 后确认语义不匹配。

```
$$\boxed{E4\text{-}0\ \text{语义闸}}:\ \text{使用任何"上位定理判 }No"\ \text{之前，必须先逐字锁定}$$
$$\qquad (a)\ \text{数据源的\textbf{问题定义}}（\text{该 }Open/No\ \text{究竟问什么}）;\quad (b)\ \text{定理的\textbf{问题定义}}（\text{约束哪个子问题}）$$
$$\qquad \text{若 }Q_{\rm sub}\neq Q_{\rm src}\ \Longrightarrow\ \boxed{\text{禁止用该定理判 }No}\ (\text{只能作为"子问题进展"记录})$$
