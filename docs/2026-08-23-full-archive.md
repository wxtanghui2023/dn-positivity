...
### 发现 6：Euler 积路径确认（17:31——唐先生补充 + 我验证）
- S₂(n) = Σ Λ(m)/m·Q_n(log m) − 正规化（无条件素数表示——不预设零点位置）
- Q_n(t) = −n·Σ C(n−1,k)(−1)^k t^k/(k+1)!——Q_n(0)=−n——主导项 (−1)^n t^{n−1}/(n−1)!
- **数值确认**：S₂(n) = O(1)（n=2: +0.96, n=5: +1.45, n=10: +1.35, n=20: +0.68——N 收敛）
- n=20 时 η_j 路径 −4.44 是精度问题（Euler 积给 +0.68——Coffey 范围一致）
- 核心方程：Σ Λ(p)/p·Q_n(log p) − 正规化 = O(1) ⟺ ψ(x) = x + O(√x) ⟺ RH
- 文档：docs/euler-product-qn-path.md
