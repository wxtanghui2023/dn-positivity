...
## 6. ⚠️ 重要澄清："2/3 零点在临界线"是条件性结果（2026-08-22 补充）

### 6.1 arXiv:2511.20059（Goldston-Suriajaya 2025-11）

**原文明确**："While at present we cannot unconditionally obtain this result"（无法无条件获得）。

精确陈述是**假设性**的：
> 若零点都在窄带 |σ−½| < b/(2logT)（b=0.3185），则 ≥2/3 简单 + ≥2/3 临界线 + ≥1/3 两者兼有

**结论**：该 "2/3" 以"零点接近临界线"为前提（RH 弱化版），**不是**非循环突破。

### 6.2 真正无条件的状态

| 结果 | 内容 | 状态 |
|:---|:---|:---|
| BGSTB24 | Montgomery F(α) 对支撑 [−1,1] 无条件 | ✅ 无条件 |
| Gallagher-Mueller | 近距离零点排斥 (0<λ≤√L) | ✅ 无条件 |
| Saker/Hall | 大间距 ≥1.99× 无穷多次 | ✅ 无条件 |
| GLSS26 | PCC ⟹ 100% 简单+临界线 | ⚠️ 推理无条件，前提 PCC 猜想 |
| **临界线比例 2/3** | Bui-Conrey-Young 2011 最佳无条件 41.05% | ❌ 2/3 是条件性的 |

### 6.3 可用技术（不依赖条件性）

- **Selberg/Fujii 去 RH 方法**（BGSTB24 2024 成功用于对关联）
- **Sylvester 惯性定律替代零点侧读取**（线性代数方法）
- 这些是真实可用的技术路线，但输出仍是条件性的
