...
## 9b. ⚠️ 限定（由 ER-1 得出；限定解读，不改写已验证事实）
ER-1（`docs/ER-1-equality-rigidity-premise.md`）证明：$\alpha_*=1/2$ 对一切 $k$ 都可由 $m=1/k^2$ 精确达到，
但 $k\ge3$ 的解 $r(1-r)=1/k^2$ **两个根都严格偏离平衡**（$k{=}3$：$0.127/0.873$；$k{=}10$：$0.010/0.990$），
而 $k{=}2$ 的唯一根 $r=1/2$ 使两个 child 都等于平衡态本身 = **对称化（ABD-1 的 (c) 插入模式）**。
⟹ 本文 §3/§8 中"**$1/2$ = 上确界且仅在平衡处达到**"的表述**须限定为**：
```
仅对【均匀 2-child】refinement 成立，且该达点退化（对称化）；
对 k≥3，α_*=1/2 由【不平衡】child ratio 达到 ⟹ 1/2 不是平衡的专属值。
```
已验证事实（$m\le1/4$ 等号 $\iff a{=}b$；特定非对称 2-child 的 $\alpha_*<1/2$ 且 $a/b\to1$ 时 $\to1/2$；
$\alpha_*=\log k/\log(1/m)$）**不受影响**。
