> ⚚⚚ **本文 §2 的圆盘公式【有代数错误，已更正】** ✗（2026-09-12 23:57，由 `A4-C-pointwise-disk-constants.md` 核验 ✓）
> · 正确（DFMR I Thm 2.2 ✓）：`rad = √(1−d²)/d²`，**圆心 `(1/d², H)`** ✓
> · 本文旧式 ✗：`rad = |λ|·√(1−d²)/(a·d²)` ⟹ **夸大 `2|λ| ≈ 2H` 倍** ✗
> · 另两处更正 ✓：① 引文编号 **Prop 7.5 → Prop 7.6** ✓；② 本文引的显式函数式是 **§7.2 的
>   Dirichlet 特征版** ✗，**ζ 版应为 `ψ(u) = {u}`（小数部分）** ✓，即 `K_r = span(t^r({α/t} − α{1/t}))` ✓
> · ⭐ **新增负面判定** ✓：`r = 1/2` 时 `d²` 随 H **饱和到 1**（H=10³ → 0.999993；H=10⁵ → 0.999999997）
>   ⟹ **圆盘半径 ~ O(1/H)**（2.6e−3 / 5.7e−5）**基本为空**，且**不跨 Re = 1/2**，
>   与 Platt–Trudgian **零冲突**、对临界线**零信息** ✓ —— 即 Burnol 自述的 "disappointing fact" ✓
>   ⟹ **E27 的点态机制在 r = 1/2 实操价值为零，且这是结构性的（`t^{−iH}` 相位退相干），与归一化无关** ✓
> （阈值 ✓：rad ≥ 1 ⟺ d² ≤ 0.618 = 1/φ ｜ ≥10 ⟺ 0.095 ｜ ≥10³ ⟺ 1e−3 ｜ ≥10⁶ ⟺ 1e−6 ✓）
