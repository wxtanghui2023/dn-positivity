...
## ⚠️ 复核与降调（2026-09-12 10:03，第 13 次自查 ✓）

```
【复核结论】**"窗口泛函饱和"这一结果【不是新发现】** ✗ —— 它是**经典最优性的重述** ✓：
   · c_MT^{−1} = 1/2 + cot(1/√2)/√2 **定义上就是** Montgomery–Taylor 泛函的**最小值** ✓✓
   · 而"**ψ_MT 在窗口类内最优**"**在我方 A3 对齐档案中已记录** ✓✓
     （原文："**optimal within window class ψ ([CCLM17 Cor. 14])**" ✓）
   ⟹ 本项的数值工作 = 对该**已知最优性**的**独立数值确认** ✓（**有价值，但不是发现** ✗）
【⟹ 应降调的表述】我此前称其为"**决定性发现**" ✗ —— 应改为：
   "**对已知最优性的独立数值确认，并明确化其后果**" ✓✓
【✅ 本项【仍然有用】的部分（保留 ✓）】
   ① 数值确认 **min R = c_MT^{−1}**（非负窗口 SLSQP ✓ + 带号窗口精确 KKT ✓，N→1600 ✓）
   ② **明确化后果** ✓：0.6725 → 0.682 的那 **0.01 来自 Christoffel/矩输入**（1 − Λ₁(0) = m₁²/m₂ ✓），
      **不来自窗口** ✓；再往 0.682 之上**需【无条件三阶矩】** ✗（= 素数幂乘法关系障碍 ✓）
   ③ ⟹ **E10 的窗口搜索提议仍被否定** ✓（这一点**不受降调影响** ✓ —— 因窗口已由经典结论最优 ✓）
【⚠️ 一般教训】"数值复现一个**已被证明**的最优性" ✗ ≠ "找到新结果" ✓ ——
   引用链须**先查**我方档案与文献（**本次即已在我方档案中** ✗✓）⟹ **查重应先做** ✓
```
