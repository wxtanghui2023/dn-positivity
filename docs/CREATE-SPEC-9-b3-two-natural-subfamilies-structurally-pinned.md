...
- ⚠️ **§1 的自我限定（本档补）**：**已证部分**＝显式公式 $\sum\Lambda^2n^{-s}=\sum_p(\log p)^2/(p^s-1)$ ＋ 收敛横标 $1/2$；**"不含零点信息"一句为 `[结构性]`**
  —— 理由：该级数在 $\sigma>1/2$ 上**绝对收敛**故在其内解析；"奇点＝$p^s=1$"是**各项极点**的陈述，
  而 $\sigma<1/2$ 的**解析延拓**性质本档**未分析** ⟹ **不得**写作"已证与 ζ 零点无关" ✓
