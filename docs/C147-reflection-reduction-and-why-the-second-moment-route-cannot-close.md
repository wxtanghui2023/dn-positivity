...
## 【更正·`C-148`】（2026-09-19 12:5x）

⚠️ §4 的"**DEAD**（结构性）"判定**过强**：其中 "$10\times$ 缺口" 来自**任意选取的** $\delta\approx0.4$；
按配置自然 $\delta$（极值配置 $\delta=\pi/3$）$c_\delta=2.5$ ⟹ 缺口降至 $\approx4$–$5\times$。
且 $\delta$-无关部分仅 $\approx2\times$ ⟹ 状态应为"**差 $2\times$、待救**"，**不是** DEAD。
（救活取决于"联合不等式"的真伪，未证。）
