...
## §5 补充：开放获取渠道探测（2026-09-26 09:59，**亦全部落空** ✓）

```
$$\text{① Semantic Scholar API （DOI 直查）}:\ \texttt{openAccessPdf.status = \textbf{CLOSED}}\ ✗\ \text{（无合法免费 PDF ✓）}$$
$$\text{② JCD 电子报告镜像 } \texttt{emba.uvm.edu/\!\!\sim\!\!jcd/reports/}:\ \textbf{主机不可达}（HTTP 000\ ✗）$$
$$\text{③ Firecrawl 检索"免费 PDF"}:\ \text{仅返回已知三页（作者站点 }8\text{-}2\text{-}12\ ✗\ /\ \text{Semantic}\ ✗\ /\ \text{ResearchGate}\ ✗）\ ✗$$
⟹ **结论**：2000 年论文**无合法免费副本** ✗；只能由唐先生经机构订阅下载 ✓
```

**唐先生下载后，本线所需的两件事（请只取这两处 ✓）**
```
$$\text{① **等价关系定义**: 论文中 "equivalence" 的确切定义}（\text{应为坐标置换+平移}=B_n\ ✓；\text{须逐字核对 ✓}）$$
$$\text{② **附录代表码表**: } n\le8,\ R=1,\ M=K(n,1)\ \text{的代表元列表}（\text{尤需 }n=6,7,8\ ✓）$$
```
