...
> **时间**：2026-09-18 13:52 唐先生：按预注册顺序执行 `U1`–`U6`（R1 已在 bounded-family 意义上坍缩）
> **性质**：证据清理；**不改任何原档**（只登记勘误与降调）✓
...
$$\text{检索（含摘要、正文部分、Nyman--Beurling 文献圈）}\ \textbf{未找到}\ \text{"What is essential nevertheless is the uniformity as }A\to0\text{"}✓$$
$$\qquad \text{同时}\ \textbf{确证}\ \text{Burnol 一文的对象}：\text{Nyman--Beurling／Báez-Duarte 逼近}\ d_n;\ \ \text{RH}\iff\lim d_n=0;\ \ \text{猜想}\ d_n^2\sim C\log n,\ C=2+\gamma-\log(4\pi);$$
$$\qquad \qquad \text{Burnol 给出}\ d_n^2\ \text{下界（同常数，改进）}✓$$
...
$$\text{`MO 444337` 转述的优化界}：\|B\|_{2,2}\le\pi\Bigl(1+(6/5)^{1/2}\Bigr)^{1/3};\qquad \text{数值自检}：(1+\sqrt{1.2})^{1/3}=(2.0954)^{1/3}=1.2795\Longrightarrow 1.2795\pi\approx4.0196✓$$
$$\Longrightarrow\ \text{"}\le1.28\pi\text{"}\ \textbf{表述正确}✓;\ \text{但}\ \textbf{来源仍为社区转述} ⟹ \text{标}\ \text{[转述·数值自检通过]}✓$$

## §4 `U4`：`PAPERA` vdC 常数——**待核但不承重**

$$\text{档内标注（逐字）}：\text{"van der Corput 二阶导检验}\（\text{定理，常数待核}）\text{"}\ \big|\ \text{"}\text{[定理：Graham--Kolesnik《van der Corput's Method》二阶导检验，常数待核]}\text{"}✓$$
$$\Longrightarrow\ \textbf{处置}：\text{标}\ \boxed{\text{待核但不承重}};\ \text{因}\ \text{`C-90`}\ \text{的判定依据＝}\textbf{定号假设实测失效}（\text{正 999942／负 1001108}），\ \textbf{与常数无关}✓✓$$

## §5 `U5`：8/23 第二环——**降调**

$$\text{档案第 30 行（逐字）}：\text{"}\textbf{反证法（无条件）}\text{：离轴}\Longrightarrow S_2\ \text{指数发散"}✓$$
$$\text{我方}：\text{C-62（窗口空洞）＋ C-63（两条候选输入全不可用：}S_2=O(1)\iff\text{RH）}✓$$
$$\Longrightarrow\ \textbf{处置}：\text{该行}\ \textbf{加注}\ \boxed{\text{"输入不可达（见 `C-62`／`C-63`）"}};\ \text{身份由"已证"降为}\ \text{[档案声称／输入不可达]}✓✓$$

## §6 `U6`：转述风险面——**产协议**

$$\text{清点}：\textbf{83 档}\ \text{含 }\ \text{"转述／二手"}\ \text{字样} ⟹ \text{全量复核不现实}✓$$
$$\textbf{协议（自本档起适用）}：$$
$$\qquad \text{(1)}\ \textbf{承重引用}（\text{被地图上任何一个"关闭"所依赖者}）\ \textbf{必须逐字＋出处可查};\ \text{否则}\ \textbf{不得} \text{承重}✓$$
$$\qquad \text{(2)}\ \textbf{非承重引用} \text{可转述，但必须显式标注}\ \text{[转述／待核]}✓$$
...
- `[外察]` §1／§2／§3 均为**检索所得**（`excerpt` 级）；§2 的 `Prop 9` 来自 `HAL` 预印本（**非正式发表**）⚠️；§7 的 Michalowski 条目为**检索摘要**级 ⚠️
- `[内察]` §4／§5／§6 为**档内逐字**与清点 ✓
- **本档不改任何原档** ✓（只登记降调与勘误）；**不声称**任何关闭为错 ✗；不证 RH ✗
- **纪律**：先查后判（R-1 ✓，**先跑后写** ✓）；**未用 RH 作推导** ✓；**零数值** ✓（`U3` 的数值为**自检算术**，非研究数值）

```
⚠️ 唐先生 13:52：按预注册顺序执行 U1–U6（R1 已在 bounded-family 意义上坍缩）
