...
## §8 结论修正（唐先生 2026-09-15 15:11；随后由 `V213` 执行）

$$\text{本档}\ §4\text{--}§5\ \text{的表述「canonical 来源只有一条产线」}\ \text{不够准确};\ \textbf{更强且更准确的表述是}：$$
$$\qquad \boxed{\text{只要机制的}\ \textbf{基本对象} \text{仍然是}\ \xi/\zeta\ \text{的}\ \textbf{同一个零集}，\ \text{它就无法产生第二个独立的}\ \beta\text{-坐标}} ✓✓✓$$
$$\qquad \text{理由}：\text{限制不在产线条数，而在}\ \textbf{对象的同一性};\ \text{同一零集}\ \Longrightarrow\ \text{同一}\ \beta\text{-坐标};\ \text{要第二个}\ \beta\text{-坐标必须}\ \textbf{换对象} ✓$$
$$\qquad ⚠️\ \text{故}\ §5\ \text{的「必落三管道」应读作「}\textbf{在同一零集内} \text{必落三通道」};\ \text{换对象的路线即}\ \text{`V213`}\ \text{所审} ✓$$
