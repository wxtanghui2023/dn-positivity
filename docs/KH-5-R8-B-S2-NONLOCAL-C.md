...
```
【判词（照录 ✓）】
　KH-5/R8-B/S2-NONLOCAL-C = CLOSED
　KH-5/R8-B/S2-NONLOCAL = CLOSED under current three-class map
　　⭐ 必须保留 under current three-class map —— 不写"非局部方法不存在"

KH-5 整线记录：S2 不是缺第三对象；有限阶无增益；
　三类非局部候选均已有档案覆盖；C 的剩余 1/3 收敛到 W6 原子墙

完整链条：
　R8 → S2 缺主项 → 有限阶聚合全部 G(K) <= 0 → 非局部三类
　→ A 零点侧已封 → B 算术相关性饱和 → C 非局部正性最多形成部分计数
　→ 2/3 基准 → 1/3 = R-1 → support>1 → W6 原子墙
　若强行 2/3 ⟹ 1：终点 n_-(Q_T)=0 ⟺ Weil positivity ⟺ RH
```

```
【两项必留资产】
　① 新机制（登记 E-11）：indefinite form → inertia → rank → zero counting
　　── 与直接 Weil 正性不同的部分计数机制，作为独立工具资产保留
　② 转移原理（登记 E-12）：二阶矩 + 惯性 ⟹ 计数下界
　　── 是 V182 的对偶面；未来若出现完全不同的 global arithmetic object，可重新调用
```

```
【规则保留】 不再继续追 S2（这已不是"还没找到技巧"，而是 R8 → S2 → nonlocal → C ⟹ W6 原子墙）
　不得为跨过 W6 原子墙而重新包装有限阶相关性/正性
【战略】 回到 E-10 正空间：寻找不以 R8/S2 为载体、具有自身独立问题和真正参数的 arithmetic object
　两个资产留作以后候选可复用的后置接口，不是新主线
```
