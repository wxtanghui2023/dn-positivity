...
> ⚚ 更正（2026-09-12，A5-2 精读 ✓）：本文 §3 的「RH ⇒ O_p(1)」【表述不成立】✗ —— 主项来自**零点密度**（**与 RH 无关** ✓），RH 只移除**离轴零点贡献** ✗，**不能移除**量级为 **log X / log p** 的振荡项 ✓（即经典 **S(T) = O(log T)** 结构 ✓；Titchmarsh §9.11：**无已知改进** ✓）。§3 的数值「O(1)」是**有限范围效应** ✓（X = 1.13×10⁶ 时 log X ≈ 14 ✓）。详见 `A5-2-phase-locking-sharpness-check.md`。
