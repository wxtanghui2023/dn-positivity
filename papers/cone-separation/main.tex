...
The resulting negative statement is deliberately formulated as a \emph{registered-framework obstruction}: within the canonical framework we specify, non-vacuity and one-sided discrimination are incompatible, and we do not claim a complete impossibility theorem; instead we isolate precisely the single unexcluded degree of freedom (a non-canonical, $\iota$-breaking Archimedean object). \end{abstract}
...
\begin{proposition}[Class of the certificate, relative to the classification used here]\label{prop:class}
The certificate in \textup{(H1)} is a positive combination, $\sum_iy_ig_i=0$ with $y>0$; this identity is a
mathematical fact ($\mathbf{F0}$). Within the classification employed in this programme ($\mathbf{F3}$), merely
renaming, re-symbolising or restating an existing positivity structure is not counted as a new positivity
mechanism.
\end{proposition}
\begin{proof}
The identity is \textup{(H1)} rewritten row-wise. The second clause is a classification convention of this
programme and is marked $\mathbf{F3}$ accordingly; it supports only the relative statement just made, and it is
not a mathematical theorem to the effect that no further positivity mechanism exists.
\end{proof}

\begin{proposition}[Canonical (universal) obstruction]\label{prop:iota}
Let $\iota$ be the involution attached to the functional equation, and suppose the datum used by a
candidate separator is \emph{canonical}, i.e.\ determined by the completion type alone and not by the zero
configuration of the object. Then the datum is $\beta$-blind: it takes the same value for every object of
the given type. Consequently a canonical construction cannot supply a separator that discriminates $\beta$,
and in particular cannot supply the one-sided, axis-breaking separator required at the interface.
\end{proposition}
\begin{proof}
By canonicity the datum is a function of the completion type only, hence constant across all objects of that
type, whereas $\beta$ varies with the object's zero configuration; a constant cannot separate two different
values of $\beta$, so no $\beta$-discriminating output arises. We do \emph{not} assume that arithmetic data
are $\iota$-invariant: $\iota$ remains the background structure of the functional equation, and no general
invariance claim about arithmetic data is asserted.
\end{proof}

\begin{proposition}[Orientation at the membership boundary]\label{prop:robin}
For the membership boundary examined, the two sides of the defining comparison involve an Archimedean constant
and an Archimedean iterated logarithm; these are mathematical facts ($\mathbf{F0}/\mathbf{F1}$). Within the
interface framework adopted in this programme ($\mathbf{F2}$), that orientation is not admissible as the
arithmetic axis-breaking interface we require. The concluding clause is $\mathbf{F2}$-relative.
\end{proposition}
\begin{proof}
Direct inspection of the defining comparison: the two sides involve $\sigma(n)$ and $e^{\gamma}n\log\log n$,
and $e^{\gamma}$, $\log\log n$ are Archimedean objects. The final clause concerns the interface framework of this
programme and is not an unconditional impossibility statement.
\end{proof}
...
\textup{(ii)} the Archimedean factors available in the canonical framework are \emph{universal} (they depend only on the completion type, not on the zero configuration of the object, and are therefore $\beta$-blind), and
...
\end{proposition}
\begin{proof}
By \textup{(i)} a non-vacuous balancing factor is Archimedean; by \textup{(ii)} it is universal, hence $\beta$-blind and unable to discriminate $\beta$-values; by \textup{(iii)} discrimination requires data that vary with the object. Contradiction. Contradiction.
\end{proof}
...
\section{Hypothesis domain}\label{sec:domain}

The obstruction statements of \S\ref{sec:scope} are relative to an explicitly classified domain. We separate
\begin{itemize}
\item[\textbf{F0}] \emph{mathematical facts}, e.g.\ the symmetry $\Lambda(s)=\Lambda(1-s)$ of the completed
object;
\item[\textbf{F1}] \emph{proved structural facts of the framework}, e.g.\ canonicity/universality of the
Archimedean factor and the resulting blindness with respect to the zero configuration;
\item[\textbf{F2}] \emph{interface and method choices}, e.g.\ selecting a separator as the interface between the
geometric and the arithmetic side;
\item[\textbf{F3}] \emph{researcher conventions}, e.g.\ declining a particular non-canonical Archimedean
construction.
\end{itemize}
Only \textbf{F0} and \textbf{F1} may enter the premises of an obstruction theorem; \textbf{F2} and \textbf{F3}
must not appear as implicit hypotheses of an impossibility statement. Accordingly we state the obstruction in
the relative form
\[
\text{the obstruction holds relative to } (\mathbf{F0},\mathbf{F1}),
\]
and we do not claim that all conceivable methods are impossible. This separation is intended as a type system
guarding against the failure mode in which a methodological choice is silently promoted to a structural fact.
