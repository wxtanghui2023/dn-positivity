# A Structural Theory of Detection without Exclusion
## 从检测到排除：Riemann 型猜想统一框架的结构性边界
...
C3: V(T) = O(T^{−1})
C4: pointwise ψ(x) − x = O(x^{1/2+ε})
```

### 4.2 逐项关系（严格推导）
- **C0**：由 PNT 无条件成立（ψ−x = O(x e^{−c√log x})——平方积分收敛）——且——任何 β_max < 1 都成立——**太弱**（不排除 ½ < β < 1）。
- **C1**：由 T7（V(T) ~ T^{2β_max−2}）——C1 ⟺ β_max < (2−a)/2——**编码零点位置**（部分信息——需零点知识）。
- **C2 ⟺ RH**（L² 版本）：ψ(x)−x = O(x^{1/2+ε})（pointwise）⟺ RH（经典）——C2 是其 L² 版本——由 T7，C2 ⟺ β_max ≤ ½（T^{2β_max−2} ≤ T^{−1+ε} 对所有 ε ⟺ 2β_max−2 ≤ −1 ⟺ β_max ≤ ½）——**等价于 RH**（零点半平面 σ > ½ 无零点）。
- **C3**：由 T7——C3 ⟺ β_max < ½——**比 RH 更强**（RH 允许 β_max = ½——在线零点）——且——若 β_max = ½ 且在线零点不抵消——C3 不成立——**C3 可能是假命题**（除非在线零点给出额外结构抵消——开放）。
- **C4 ⟺ RH**（经典等价）。
...
C0 ≪ C1 < C2 ~ RH ≤ C3, C4
```
- C0 ≪ C1：C1 需要 β_max < (2−a)/2 < 1——严格强于 C0。
- C1 < C2：C2 需要 β_max ≤ ½——严格强于 C1（a=1 时 C1 只给 β_max < ½+ε'）。
- C2 ~ RH：等价（L² 版本）。
- RH ≤ C3：C3 ⟹ β_max < ½ ⟹ RH；但 RH 不 ⟹ C3（β_max = ½ 允许）。
- C2 < C4：C4（pointwise）⟹ C2（L²）——C2 不 ⟹ C4。

### 4.4 谱系的含义
**越想让 observable 真正"排除"离轴零点，就越接近 RH 本身。** C2（临界尾部）与 RH 是同一逻辑强度——任何"从素数侧推出 C2"的尝试等价于证明 RH。这是"为什么体积探针只能检测"的精确表述。
