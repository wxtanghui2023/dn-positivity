...
For $\lambda=k/H^{2}$, with $v(t)=\tfrac k2\log(1+t^{-2})$ and $F$ as in Lemma~\ref{lem:mono},
\[
\mathrm{near}\;\le\;\frac1\pi\int_H^{2H}4\sinh^{2}\bigl(\tfrac12v(t)\bigr)\log\frac t{2\pi}\,dt,
\]
and equally, integrating by parts against the counting function so that only the constants of
\eqref{eq:H} enter,
\[
\mathrm{near}\;\le\;F(2H)\,M(2H)-F(H)\,M(H)+\int_H^{2H}|F'(t)|\,M_{\mathrm{up}}(t)\,dt,
\qquad M_{\mathrm{up}}(t)=2\bigl(at\log t+bt+c\log t+d\bigr).
\]
\end{lemma}

The second form is the one to be used in a fully explicit write-up: the pointwise local count derived
from \eqref{eq:H} has leading constant $a+2c$, about $2.4$ times the smooth density's $a$, and would
push the ratio in the critical band above one, whereas the integrated form keeps the sharpness by
cancelling the boundary contribution in the same way as in \S\ref{sec:abel}. Verified numerically,
the second form gives a ratio of at most $0.351$ across the band, marginally better than the first.
...
\item The local count in Lemma~\ref{lem:near} is now available in the explicit Abel form recorded in
that lemma, whose only inputs are the constants of \eqref{eq:H}; it was verified numerically to give a
ratio of at most $0.351$ across the critical band, so this item is closed. The version using the actual
zeros in place of the count gives the same ratio to three decimals, an independent check.
