...
% Check 1-3：无"verified numerically"措辞——引用精确——Limitations Statement
...
\title{A Weil-Explicit-Formula Criterion via a Positive Spectral Discrepancy\\[0.5em]
\large Conditional on the stated analytic framework}
\author{}
\date{}
...
\section{Weil Interface}

\noindent\textbf{Test space.}
\begin{equation}
W=\{H\in\mathcal S'(\mathbb R): H(-t)=H(t),\ H\in C^\infty,\
|\widehat H(u)|\le C(1+|u|)^{-1-\epsilon}\}.
\end{equation}
The exponential decay $|\widehat H_0(u)|\le Ce^{-|u|}$ satisfies this
admissibility condition.  We adopt the $t$-variable formulation of Weil's
explicit formula, in which the zero term reads
$Z(H)=\sum_\rho H(\gamma_\rho)$ over all nontrivial zeros counted with
multiplicity.

\begin{proposition}[Weil compatibility]\label{prop:weil}
...
$Z(H_0)=\sum_\rho H_0(\gamma_\rho)$.
\end{proposition}

\noindent Verification: $H_0(t)=-\tfrac12\bigl[\tfrac1{2\pi}\log(1+t^2)+
\tfrac1{\pi(1+t^2)}\bigr]$ is even, real, $C^\infty$, and tempered;
$\widehat H_0(u)=-\tfrac12(1+|u|)e^{-|u|}/|u|$ decays exponentially.
The present work does not reprove the explicit formula; it verifies that
$H_0$ satisfies the required hypotheses.
...
and the applicability of the invoked explicit formula.  The equivalence
established here is conditional on the Weil--Barner interface and the
distributional identities of Appendices~B--C; independent verification of
the key lemmas by the community is required before any stronger claim is
made.
